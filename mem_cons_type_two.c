...
void usage(char *program){
    printf("Usage: \n");
	printf(" -s   <buf_size>  buffer size, default is %d\n", BUF_SIZE);
    ...
    for (int i = 1; i < argc; ++i){
        if (strlen(argv[i]) == 2 && strcmp(argv[i], "-t") == 0){
            ...
            if(i+1 < argc){
                ...
                if(buf_size <= 0){
                    printf("invalid numbers of buf_size\n");
                    exit(EXIT_FAILURE);
                }
                i++;
            }else {
                printf("cannot read numbers of buf_size\n");
                usage(argv[0]);
                exit(EXIT_FAILURE);
            }
        }
...
void *memory_consumer(void* arg){
	...
	char *a, *b, *c;
	a = (char*) numa_alloc_onnode(buf_size, numa_of_memory);
	b = (char*) numa_alloc_onnode(buf_size, numa_of_memory);
	c = (char*) numa_alloc_onnode(buf_size, numa_of_memory);
	while (1)
	{
		memcpy(a, b,  buf_size);
		memcpy(b, c,  buf_size);
	...
	numa_free(a, buf_size);
	numa_free(b, buf_size);
	numa_free(c, buf_size);
}
...
int main(int argc, char *argv[]){
	...
	printf("buffer size is %d\nnumber of threads is %d\nnuma of threads is %d\nnuma of memory is %d\n", 
			buf_size, threads, numa_of_threads, numa_of_memory);
